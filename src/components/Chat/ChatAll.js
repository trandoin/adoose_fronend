...
export default class ChatAll extends Component {
  ...
  componentDidMount = async () => {
    ...
    socket.on("meInChat", (id) => {
      ...
      socket.emit("saveUsernameInChat", {
        id: id,
  ...
  improveChat = (chats) => {
    ...
    for (let i = 0; i < chats.length; i++) {
      let newChat = {};
      if (chats[i]["P1"] != localStorage.getItem("Username"))
        newChat["person"] = chats[i]["P1"];
      if (chats[i]["P2"] != localStorage.getItem("Username"))
        newChat["person"] = chats[i]["P2"];
  ...
  ChatItems = (data) => {
    ...
    const LeftSide = data.chatAll.map((item, index) => (
      <div
        key={index}
        className="d-flex align-items-center my-1 sideLeftChatOnHover py-1 px-2"
        onClick={() => {
          this.setState({ selectedChatIndex: index }); // setting index
          if (
            item["Unread"] > 0 &&
            item["UnreadFor"] === localStorage.getItem("Username")
          ) {
      ...
      >
        <div style={{ width: "3rem", height: "3rem", marginLeft: "1rem" }}>
          <img
            src={`https://firebasestorage.googleapis.com/v0/b/adoose-cea6c.appspot.com/o/${item["person"]}%2FprofilePic%2Fimage?alt=media`}
            width="100%"
            height="100%"
        ...
        item["UnreadFor"] === localStorage.getItem("Username") ? (
          <b>
            <span style={{ fontWeight: "900", marginLeft: "1rem" }}>
              {item["person"]}
            </span>
          </b>
        ) : (
        ...
        item["Unread"] > 0 &&
        item["UnreadFor"] === localStorage.getItem("Username") ? (
          <Label className="ms-auto" size="mini" circular color={"red"}>
            {item["Unread"]}
          </Label>
        ) : null}
      </div>
      ...
      <div>
        ...
        <div className="d-flex">
          ...
          >
            ...
            <SearchChats
              addChatToList={this.addChatToList}
              people={this.state.chatAll ? this.state.chatAll : []}
            />
          ...
          <div style={{ width: "80%", flexGrow: "1" }}>
            {this.state.selectedChatIndex === -1 ? (
              ...
              <SelectedChat
                chat={this.state.chatAll[this.state.selectedChatIndex]}
